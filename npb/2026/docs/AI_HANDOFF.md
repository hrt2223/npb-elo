...
- `fetch_results_2026.py --month <月> --merge-existing` は、対象月の取得が成功し、かつ取得済み試合数が既存CSVより減っていないことを確認してから既存行を入れ直す。
- 通信失敗やパース異常で試合数が減る場合は、CSVを書き換えずにエラー終了する。これにより、取得障害で当月の過去結果を消す事故を防ぐ。
